...
import { fetchOutreachById } from "../../lib/api";
import { createOutreach, updateOutreach } from "../../lib/adminApi";
...
export default function OutreachForm() {
  ...
  const close = () => navigate("/admin/outreach");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await updateOutreach(id, form, imageFile);
      } else {
        await createOutreach(form, imageFile);
      }
  ...
  if (loading) {
    return (
      <FormSheet title="Edit Outreach Entry" onClose={close}>
  ...
  return (
    <FormSheet
      title={isEdit ? "Edit Outreach Entry" : "Add Outreach Entry"}
      onClose={close}
      footer={
        <>
    ...
    >
      ...
      <form id="outreach-form" onSubmit={handleSubmit} className="space-y-6">
